...
describe("<DateRangeInput>", () => {
    ...
    describe("default behavior", () => {
        ...
        it("has a start-date field, an end-date field, and an icon button");
        it("has no selected date range by default");

        describe("popover", () => {
            it("if closed, opens when the start-date input receives focus");
            it("if closed, opens when the end-date input receives focus");
            it("if closed, opens when the trigger button is clicked");
            it("if open, closes when the trigger button is clicked");
        });

        describe("placeholder text", () => {
            it("shows 'Start Date' in the start-date field when the field isn't focused");
            it("shows 'End Date' in the end-date field when the field isn't focused");
            it("shows the formatted minDate in the start-date field when the field is focused");
            it("shows the formatted maxDate in the end-date field when the field is focused");
        });

        describe("selection", () => {
            it("when a date range is selected from the picker, renders the dates in the input fields");
        });

        describe("validation", () => {
            it("shows an error message in the start-date field when a malformatted date is inputted");
            it("shows an error message in the end-date field when a malformatted date is inputted");
            it("shows an error message in the start-date field when an inputted date precedes the minDate");
            it("shows an error message in the end-date field when an inputted date exceeds the maxDate");
            // tslint:disable-next-line:max-line-length
            it("shows an error message in the end-date field when a selected end date precedes the selected start date");
            it("shows an error message in the end-date field when a selected start date exceeds the selected end date");
        });

        describe("callbacks", () => {
            it("triggers the onChange callback when a date is typed");
            it("triggers the onChange callback when a date is selected from the picker");
            it("triggers the onError callback when an invalid date is typed");
            it("triggers the onError callback when an out-of-range date is typed");
            it("triggers the onError callback date when an invalid end date is typed");
            it("triggers the onError callback date when an invalid end date is selected in the picker");
        });
    });
    ...
    describe("configurable behavior", () => {
        ...
        describe("allowSingleDayRange", () => {
            it("if true, allows a single-day range to be selected");
        });

        describe("allowUnboundedDateRange", () => {
            it("shows 'All before' in the unfocused start-date field if the selected date range is [null, <end date>]");
            it("shows 'All after' in the unfocused end-date field if the selected date range is [<start date>, null]");
        });

        describe("disabled", () => {
            it("if true, disables the start-date field");
            it("if true, disables the end-date field");
            it("if true, disables the trigger button and its surrounding phantom input field");
        });

        describe("defaultValue", () => {
            it("renders initially with the default value populated in the start- and end-date fields");
        });

        describe("format", () => {
            it("properly formats the selected start date in the start-date field");
            it("properly formats the selected end date in the start-date field");
            it("properly formats the minDate placeholder text on focus in the start-date field");
            it("properly formats the maxDate placeholder text on focus in the end-date field");
        });

        describe("invalidDateMessage", () => {
            it("if provided, shows a custom invalidDateMessage");
            it("if null, does not show an invalidDateMessage at all");
        });

        describe("invalidEndDateMessage", () => {
            it("if provided, shows a custom invalidEndDateMessage");
            it("if null, does not show an invalidEndDateMessage at all");
        });

        describe("openOnFocus", () => {
            it("if false, popover does not open when start-date field is focused");
            it("if false, popover does not open when end-date field is focused");
        });

        describe("outOfRangeMessage", () => {
            it("if provided, shows a custom outOfRangeMessage");
            it("if null, does not show an outOfRangeMessage at all");
        });

        describe("popoverPosition", () => {
            it("if provided, renders the popover in a custom position");
        });

        describe("selectedAllOnFocus", () => {
            it("if true, selects all text in the start-date field on focus");
            it("if true, selects all text in the end-date field on focus");
        });

        describe("shortcuts", () => {
            it("if provided, renders a custom list of shortcuts in the picker");
        });

        describe("value", () => {
            it("if provided, renders the custom date-range value in the start- and end-date fields");
            it("if null, renders empty start- and end-date fields");
        });
    });
